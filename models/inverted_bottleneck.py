...
class ConvNext(nn.Module):
    ...
    def _make_layer(self, block: Type[InvertedBottleneck], planes: int, num_blocks: int, stride: int = 1) -> nn.Sequential:
        ...
        # NOTE: Change of output planes for each block, gives planes instead of
        # planes*expansion. The expansion is now inside the block (inverted bottleneck).
        # This causes some changes here and there (tracking of self.inplanes,
        # projection layer, final fc layer).
