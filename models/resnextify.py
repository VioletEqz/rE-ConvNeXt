...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        # NOTE: Now uses depthwise convolution
        self.conv2 = conv3x3(planes, planes, stride, depthwise=True)
...
def _convnext(block: Type[Bottleneck], layers: List[int], width: List[int], **kwargs: Any) -> ConvNext:
    model = ConvNext(block, layers, width, **kwargs)
    return model


# NOTE: New layer widths
def convnext_t(**kwargs: Any) -> ConvNext:
    return _convnext(Bottleneck, [3, 3, 9, 3], [96, 192, 384, 768], **kwargs)

def convnext_s(**kwargs: Any) -> ConvNext:
    return _convnext(Bottleneck, [3, 3, 27, 3], [96, 192, 384, 768], **kwargs)

def convnext_b(**kwargs: Any) -> ConvNext:
    return _convnext(Bottleneck, [3, 3, 27, 3], [128, 256, 512, 1024], **kwargs)

def convnext_l(**kwargs: Any) -> ConvNext:
    return _convnext(Bottleneck, [3, 3, 27, 3], [192, 384, 768, 1536], **kwargs)

def convnext_xl(**kwargs: Any) -> ConvNext:
    return _convnext(Bottleneck, [3, 3, 27, 3], [256, 512, 1024, 2048], **kwargs)
